...
export const executeUnzipRepo = async ({
  ...
  try {
    ...
    const extractDirPath = path.join(process.env.UNZIP_OUTPUT_DIR || '/tmp', `extracted_${timestamp}`);
    const zipFilePath = path.join(process.env.UNZIP_OUTPUT_DIR || '/tmp', `repo_${timestamp}.zip`);
...
export const createUnzipRepoTool = async () => {
  ...
  return tool({
    name: 'unzip_repo',
    description: 'Scarica un repository .zip da S3 e lo decompone nel file system temporaneo locale.',
