...
const callback = async ({ basePath }: z.infer<typeof FindDocumentationFilesInput>): Promise<string> => {
  try {
...
export const createFindDocumentationFilesTool = async () => {
  ...
  return tool({
    ...
    description: 'Esplora la cartella locale del repository e restituisce i percorsi assoluti dei file di documentazione (.md, .txt, cartella docs/, CHANGELOG, README). Richiede il percorso locale restituito da unzip_repo.',
