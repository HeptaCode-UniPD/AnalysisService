...
async function runRepomix(
  ...
  try {
    ...
    console.error(
      `[SmartBundler] Errore creazione bundle ${outputSuffix}:`,
      err?.message,
    );
    return '';
  }
...
function splitIntoChunks(
  content: string,
  chunkSize: number = MAX_BUNDLE_CHARS,
): string[] {
  ...
  console.log(
    `[SmartBundler] Bundle spezzato in ${chunks.length} chunk (chunkSize: ${chunkSize} chars)`,
  );
  return chunks;
}
...
export async function createSourceBundle(extractPath: string): Promise<string> {
  ...
  if (raw.length > MAX_BUNDLE_CHARS) {
    console.warn(
      `[SmartBundler] source bundle grande (${raw.length} chars) — usa createSourceChunks per analisi completa`,
    );
  }
...
export function extractImportedLibraries(
  sourceChunks: string | string[],
): string[] {
  const libs = new Set<string>();
  const content = Array.isArray(sourceChunks)
    ? sourceChunks.join('\n')
    : sourceChunks;
