...
const UnzipRepoInput = z.object({
  ...
  zipKey: z.string().describe('La chiave (percorso) del file .zip nel bucket S3'),
});

export const unzipRepoToTemp = async (bucket: string, zipKey: string): Promise<string> => {
  ...
  if (!response.Body) throw new Error('Il file scaricato è vuoto o inesistente.');
...
export const createUnzipRepoTool = async () => {
  ...
  return tool({
    name: 'unzip_repo',
    description: 'Scarica un repository .zip da S3 e lo decompone nel file system temporaneo locale. Restituisce il percorso locale della cartella estratta, es. /tmp/extracted_123456.',
    inputSchema: {
      type: 'object',
      properties: {
        bucket: { type: 'string', description: 'Il nome del bucket S3, es. my-bucket' },
        zipKey: { type: 'string', description: 'La chiave del file .zip nel bucket S3, es. repos/job-123/source.zip' },
      },
